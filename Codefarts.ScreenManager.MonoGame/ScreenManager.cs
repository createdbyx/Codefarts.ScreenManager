...
namespace Codefarts.ScreenManager.MonoGame
{
    /// <summary>
    ...
    //        string fontAsset;
//           var settingsProvider = this.game.Services.GetService<ISettingsProvider>();
  //          fontAsset = settingsProvider?.GetSetting("GameScreenMenuFont", "menufont");

      //      this.defaultFont = content.Load<SpriteFont>(fontAsset); //"menufont");
            //this.blankTexture = content.Load<Texture2D>("blank");
            var textureSize = 32;
            this.blankTexture = new Texture2D(this.GraphicsDevice, textureSize, textureSize, false, SurfaceFormat.Color);
            var pixelData = Enumerable.Repeat(Color.Black, textureSize * textureSize).ToArray();
            this.blankTexture.SetData(pixelData);
